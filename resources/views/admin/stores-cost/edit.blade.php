...
@section('title', __('translations.edit_store_cost'))

@section('content_header')
    <h1>{{ __('translations.edit_store_cost') }}</h1>
@stop
            ...
            <div class="col-md-9">
                <div class="card">
                    <div class="card-body">
                        ...
                        <form method="POST" action="{{ url('/admin/stores-cost/' . $storescost->id) }}" accept-charset="UTF-8" class="form-horizontal" enctype="multipart/form-data">
                            {{ method_field('PATCH') }}
                            {{ csrf_field() }}

                            <div class="form-group {{ $errors->has('stores_cost_type_id') ? 'has-error' : ''}}">
                                <label for="stores_cost_type_id" class="control-label">{{ __('translations.store_cost_type_id') }}*</label>
                                <select name="stores_cost_type_id" class="form-control" id="stores_cost_type_id" >
                                  ...
                                  @foreach($storesCostTypes as $storesCostType)
                                    <option value="{{$storesCostType->id}}" {{$storesCostType->id == $storescost->stores_cost_type_id ? 'selected': ''}}>{{$storesCostType->name}}</option>
                                  @endforeach
                            ...
                            <div class="form-group {{ $errors->has('store_id') ? 'has-error' : ''}}">
                                ...
                                <select name="store_id" class="form-control" id="store_id" >
                                  ...
                                  @foreach($stores as $store)
                                    <option value="{{$store->id}}"{{$store->id == $storescost->store_id ? 'selected' : ''}}>{{$store->name}}</option>
                            ...
                            @include ('admin.stores-cost.form', ['formMode' => 'edit'])
