...
@section('title', __('translations.cancel_order'))

@section('content_header')
    <h1 class="header">{{ __('translations.cancel_order') }}</h1>
@stop
@section('content')
  <div class="container">
      <div class="row">
          ...
          <div class="col-md-9">
              <div class="card">
                  <div class="card-body">
                      ...
                      <form method="POST" action="{{ url('/admin/orders/cancel/' . $order->id) }}" accept-charset="UTF-8" class="form-horizontal" enctype="multipart/form-data">
                          {{ method_field('PATCH') }}
                          {{ csrf_field() }}
                          <div class="form-group {{ $errors->has('cancel_reason') ? 'has-error' : ''}}">
                              ...
                              <select name="cancel_reason" class="form-control" id="cancel_reason" >
                                ...
                                @foreach($order->allOrdersToCanceled($order->id) as $order)
                                  <option value="{{$order->process_number}}">{{$order->process_number}}</option>
                                @endforeach
