<div class="form-group {{ $errors->has('price') ? 'has-error' : ''}}">
    ...
    <input class="form-control" name="price" type="number" id="price" value="{{ isset($indirectcost->price) ? $indirectcost->price : old('price')}}" >
    {!! $errors->first('price', '<p class="help-block">:message</p>') !!}
</div>
<div class="form-group {{ $errors->has('reason') ? 'has-error' : ''}}">
    <label for="reason" class="control-label">{{ __('translations.reason') }}</label>
    <textarea class="form-control" rows="5" name="reason" type="textarea" id="reason" >{{ isset($indirectcost->reason) ? $indirectcost->reason : old('reason')}}</textarea>
