...
@section('title', __('translations.edit_indirect_cost'))

@section('content_header')
    <h1>{{ __('translations.edit_indirect_cost') }}</h1>
@stop
            ...
            <div class="col-md-9">
                <div class="card">
                    <div class="card-body">
                        ...
                        <form method="POST" action="{{ url('/admin/indirect-cost/' . $indirectcost->id) }}" accept-charset="UTF-8" class="form-horizontal" enctype="multipart/form-data">
                            {{ method_field('PATCH') }}
                            {{ csrf_field() }}

                            <div class="form-group {{ $errors->has('indirect_cost_type_id') ? 'has-error' : ''}}">
                                <label for="indirect_cost_type_id" class="control-label">{{ __('translations.indirect_cost_type_id') }}*</label>
                                <select name="indirect_cost_type_id" class="form-control" id="indirect_cost_type_id" >
                                  ...
                                  @foreach($indirectCostTypes as $indirectCostType)
                                    <option value="{{$indirectCostType->id}}"{{$indirectCostType->id == $indirectcost->indirect_cost_type_id ? 'selected' : ''}}>{{$indirectCostType->name}}</option>
                                  @endforeach
                            ...
                            @include ('admin.indirect-cost.form', ['formMode' => 'edit'])
