...
@section('title', __('translations.edit_material'))

@section('content_header')
    <h1>{{ __('translations.edit_material') }}</h1>
@stop
@section('content')
    <div class="container">
        <div class="row">
            ...
            <div class="col-md-9">
                <div class="card">
                    <div class="card-body">
                        ...
                        <form method="POST" action="{{ url('/admin/materials/' . $material->id) }}" accept-charset="UTF-8" class="form-horizontal" enctype="multipart/form-data">
                            {{ method_field('PATCH') }}
                            {{ csrf_field() }}
                            <div class="form-group {{ $errors->has('material_type_id') ? 'has-error' : ''}}">
                                <label for="material_type_id" class="control-label">{{ __('translations.material_type') }}*</label>
                                <select name="material_type_id" class="form-control" id="material_type_id" >
                                  <option value="" selected disabled>{{ __('translations.select_material_type') }}</option>
                            ...
                            <div class="form-group {{ $errors->has('quantity_type') ? 'has-error' : ''}}">
                              ...
                              <select name="quantity_type" class="form-control" id="quantity_type" >
                                <option selected disabled>{{__('translations.select_quantity_type')}}</option>
                                  <option value="0" {{$material->quantity_type == 0 ? 'selected' : ''}}>{{ __('translations.kilo') }}</option>
                                  <option value="1" {{$material->quantity_type == 1 ? 'selected' : ''}}>{{ __('translations.piece') }}</option>
                             </select>
                            ...
                            <div class="form-group {{ $errors->has('type') ? 'has-error' : ''}}">
                                ...
                                <select name="type" class="form-control" id="type" >
                                  <option selected disabled>{{__('translations.select_type')}}</option>
                                    <option value="0" {{$material->type == 0 ? 'selected' : ''}}>{{ __('translations.primary') }}</option>
                                    <option value="1" {{$material->type == 1 ? 'selected' : ''}}>{{ __('translations.secondary') }}</option>
                               </select>
